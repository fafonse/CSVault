...
            body.get("merchant_bank"),

            body.get("merchant_bank_acct"),
...
    if "joseph" in bank_name:  # lazy/manual input fixing
...
## Transaction Reports
Due to time constraints, I wasn't able to create a visual report of my data. However, I was able to show the current state of the API with the faulty banks.

My only regret is that my code does not do equal amounts of each bank, but instead randomly draws from a large pool of the test accounts. This leads to certain leanings towards banks with larger test account pools. I would remedy this in the future if given more time.
